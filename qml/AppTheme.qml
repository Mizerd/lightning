...
QtObject {
    ...
    // ── AND LIGHTNESS WAS THE ONLY CUE THEY HAD ───────────────────────
    //
    // Reported 2026-09-19: "hard to tell the layers apart". MEASURED, and the
    // ladder was doing exactly one thing: rung to rung it moved 3.4 ΔL* and
    // the CHROMA went 4.87 -> 4.63, i.e. flat and very slightly DOWN. Every
    // boundary in the rail was a brightness step and nothing else, because
    // `text` is a near-neutral and tinting a near-neutral rail toward it can
    // only produce greys.
    //
    // So each rung's ink is now mixed toward the theme's OWN `accent` by a
    // growing fraction. THE LIGHTNESS LADDER IS UNCHANGED — the alphas are
    // re-solved so every rung lands on the same L* it did before (7.35, 8.35,
    // 11.75, 15.15, 18.55 on Lightning Dark) — so nothing the ceiling round
    // argued for moves. What changes is that depth now carries a SECOND,
    // independent cue: chroma climbs 5.2 -> 16.5, and the perceptual distance
    // between adjacent regions goes from a flat 3.4 ΔE to 3.9 / 4.9 / 6.5 at
    // the depths where the rail is busiest.
    //
    // Deliberately the accent and not a new colour: it follows all eleven
    // presets for free and the rail reads as part of the theme rather than as
    // a blue-tinted exception. Kept small — the deepest region is #222c44, a
    // muted navy, and the SELECTION language is untouched because that is a
    // saturated 2px STROKE plus an `accentSoft` fill on the tile, a different
    // device from a wash three shades off the rail's own ground.
    ...
    // WRITTEN OUT, not generated by a function: a binding that reached these
    // colours through a call would not register a dependency on them, so a
    // theme change would leave the rail painted in the previous palette.
