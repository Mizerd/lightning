...
| Spell checking | v0.9: the operating system's own checker (Windows `ISpellChecker`, macOS `NSSpellChecker`, Linux enchant-2 via `dlopen`). Every lookup is local and per word; no draft, word, room name or Matrix id is sent anywhere, nothing is logged, and only the preference (on/off, language tag) is persisted. "Add to dictionary" writes the USER'S platform dictionary by their explicit choice; "Ignore" is session memory. See `docs/spell-checking.md`. | None planned |
