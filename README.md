...
[![Latest release](https://img.shields.io/badge/release-v0.9.6-2f6be0.svg)](https://gitlab.smetonis.net/Mizerd/lightning/-/releases)
...
Replace `0.9.6` below with the version you downloaded.
...
```sh
sudo apt install ./lightning_0.9.6_amd64.deb            # Debian 13+
sudo dnf install ./lightning-0.9.6-1.x86_64.rpm         # Fedora 44+
sudo zypper install ./lightning-0.9.6-1.x86_64.rpm      # openSUSE (untested)
...
# replaced.
chmod +x Lightning-0.9.6-x86_64.*pp[Ii]mage && ./Lightning-0.9.6-x86_64.*pp[Ii]mage

flatpak remote-add --if-not-exists --user flathub https://flathub.org/repo/flathub.flatpakrepo
flatpak install --user flathub org.kde.Platform//6.9     # the runtime, once
flatpak install --user ./lightning_0.9.6_amd64.flatpak
flatpak run org.lightning_matrix.Lightning

sudo snap install --dangerous ./lightning_0.9.6_amd64.snap
```
...
```nix
{
  inputs = {
    ...
    lightning-matrix-client = {
      ...
      #url = "github:Mizerd/lightning/v0.9.6"; # Use this if you want a specific version
    };
  };
  . . . # Your outputs config
}
...
(`Get-FileHash .\Lightning-0.9.6-<sha>-windows-x86_64.msi -Algorithm SHA256`),
